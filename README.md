...
仓库地址：[Trav011er-star/LeetCode](https://github.com/Trav011er-star/LeetCode)
...
| ---: | --- | --- | --- | --- | --- |
| 1 | [两数之和](./01-两数之和.py) | 简单 | 数组、哈希表、`enumerate()` | $O(n)$ | $O(n)$ |
| 49 | [字母异位词分组](./49-字母异构词分组.py) | 中等 | 字符串排序、哈希表、列表 | $O(nk\log k)$ | $O(nk)$ |
...
源码：[49-字母异构词分组.py](./49-字母异构词分组.py)
...
为了让变量含义更清楚，README 中使用 `word` 表示当前字符串。仓库当前源码使用的是 `str`，代码仍然可以运行，但 `str` 是 Python 内置的字符串类型名称，后续更适合将它改成 `word`，避免覆盖内置名称。
